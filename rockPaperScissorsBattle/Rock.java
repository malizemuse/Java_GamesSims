...
public class Rock {
	...
	private int endurance = 4; // Sets endurance
	private int restReq = 1; // Sets rest required to recover endurance
	...
	protected void rockAct() {
		...
		escFromPaper(); // If pred is close enough, go in the opposite direction
		getToScissors(); // Chases down prey
	...
	protected void reinforce() {
		...
		if(spawnTimer >= 33 && Simulation.numMoves <= 100
		&& locations.size() < initNumObj/2 && locations.size() > 0) {
			...
			System.out.println("        Rock army called in reinforcements!");
	...
	protected void escFromPaper() {
		
		List<Integer> predToEsc = new ArrayList<Integer>(findNearestPaper());
		...
		for(int chase = 0; chase < predToEsc.size(); chase++) {
			...
			List<Integer> pred = new ArrayList<Integer>(Paper.locations.get(predToEsc.get(chase)));
	...
	private List<Integer> findNearestPaper() {
		...
		List<List<Integer>> predList = new ArrayList<List<Integer>>(findDistsFromPaper());
	...
	private List<List<Integer>> findDistsFromPaper() {
		...
		for(int xPrey = 0; xPrey < (locations.size()); xPrey++) {
			...
			for(int yPrey = 0; yPrey < (locations.get(xPrey).size()); yPrey++) {
				...
				for(int xPred = 0; xPred < (Paper.locations.size()); xPred++) {
					
					int coordPred = Paper.locations.get(xPred).get(yPrey);
	...
	protected void getToScissors() {
		
		List<Integer> preyToChase = new ArrayList<Integer>(findNearestScissors());
		
		// Loop through pred prey pairs
		for(int chase = 0; chase < preyToChase.size(); chase++) {
			
			List<Integer> pred = new ArrayList<Integer>(locations.get(chase));
			List<Integer> prey = new ArrayList<Integer>(Scissors.locations.get(preyToChase.get(chase)));
	...
	// Finds nearest pred
	private List<Integer> findNearestScissors() {
		
		List<List<Integer>> preyList = new ArrayList<List<Integer>>(findDistsFromScissors());
	...
	private List<List<Integer>> findDistsFromScissors() {
		...
		for(int xPred = 0; xPred < (locations.size()); xPred++) {
			...
			for(int yPred = 0; yPred < (locations.get(xPred).size()); yPred++) {
				...
				for(int xPrey = 0; xPrey < (Scissors.locations.size()); xPrey++) {
					
					int coordPrey = Scissors.locations.get(xPrey).get(yPred);
