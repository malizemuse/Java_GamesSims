...
public class Paper {
	...
	private int endurance = 6; // Sets endurance
	private int restReq = 2; // Sets rest required to recover endurance
	...
	protected void paperAct() {
		...
		escFromScissors(); // If pred is close enough, go in the opposite direction
		getToRock(); // Chases down prey
	...
	protected void reinforce() {
		...
		if(spawnTimer >= 25 && Simulation.numMoves <= 100
		&& locations.size() < initNumObj/2 && locations.size() > 0) {
			...
			System.out.println("        Paper army called in reinforcements!");
	...
	protected void escFromScissors() {
		
		List<Integer> predToEsc = new ArrayList<Integer>(findNearestScissors());
		...
		for(int chase = 0; chase < predToEsc.size(); chase++) {
			...
			List<Integer> pred = new ArrayList<Integer>(Scissors.locations.get(predToEsc.get(chase)));
	...
	private List<Integer> findNearestScissors() {
		...
		List<List<Integer>> predList = new ArrayList<List<Integer>>(findDistsFromScissors());
	...
	private List<List<Integer>> findDistsFromScissors() {
		...
		for(int xPrey = 0; xPrey < (locations.size()); xPrey++) {
			...
			for(int yPrey = 0; yPrey < (locations.get(xPrey).size()); yPrey++) {
				...
				for(int xPred = 0; xPred < (Scissors.locations.size()); xPred++) {
					
					int coordPred = Scissors.locations.get(xPred).get(yPrey);
	...
	protected void getToRock() {
		
		List<Integer> preyToChase = new ArrayList<Integer>(findNearestRock());
		
		// Loop through pred prey pairs
		for(int chase = 0; chase < preyToChase.size(); chase++) {
			
			List<Integer> pred = new ArrayList<Integer>(locations.get(chase));
			List<Integer> prey = new ArrayList<Integer>(Rock.locations.get(preyToChase.get(chase)));
	...
	private List<Integer> findNearestRock() {
		...
		List<List<Integer>> preyList = new ArrayList<List<Integer>>(findDistsFromRock());
	...
	private List<List<Integer>> findDistsFromRock() {
		...
		for(int xPred = 0; xPred < (locations.size()); xPred++) {
			...
			for(int yPred = 0; yPred < (locations.get(xPred).size()); yPred++) {
				...
				for(int xPrey = 0; xPrey < (Rock.locations.size()); xPrey++) {
					
					int coordPrey = Rock.locations.get(xPrey).get(yPred);
